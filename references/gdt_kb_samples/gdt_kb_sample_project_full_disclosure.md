...
把 `open_params` 代入：`项目名 = 示例项目`，`年份 = 2025`：

- `mapping_table` 解析为：
  - `proposal/示例项目/建议书.md`
  - `implementation/示例项目/执行计划.md`
  - `monitoring/示例项目/2025_季度进展.md`
  - `evaluation/示例项目/终期评估.md`
  - `finance/示例项目/2025_收支明细.md`
...
- **动作**：用 SAG 检索 API 验证——`curl -X POST http://127.0.0.1:4173/search -d '{"query":"示例项目 从立项到结项的材料","top_k":5}'` 跑一次；
...
| 用户问法 | 模式 | 反应 |
|---------|------|------|
| "把示例项目立项到结项材料都找出来" | **GDT 触发**（命中 `question_examples`）| 加载本契约，`source_scope` 受治理检索、必带出处、无命中 BLOCKED |
